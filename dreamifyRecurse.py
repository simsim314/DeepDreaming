...
def savearray(a, filename, fmt="JPEG", quality=100):
...
def ProccessImage(img, layer, idx, output):
	layerF = layer.replace("/", "_")
	_=deepdream(net, img, end=layer)
	savearray(_,  output + "_" + str(idx) + "_" + layerF + ".jpg")

i = 0 
for l in xrange(0, int(sys.argv[4])): 
	layer = layers[int(sys.argv[3])]
	ProccessImage(img, layer,i, output)
	layerF = layer.replace("/", "_")
